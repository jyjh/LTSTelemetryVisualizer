function writePlotlyHtml(outputHtml, simRun, realRun, track, comparison, summary, sceneFile)
...
sceneLinkHtml = sceneLink(sceneFile);
sceneLinkStyle = ['a.sceneLink{display:inline-block;margin-top:8px;padding:6px 12px;' ...
    'background:#1d3358;color:#cfe0ff;border:1px solid #4f8cff;border-radius:6px;' ...
...
html = sprintf(['<!doctype html>\n<html>\n<head>\n<meta charset="utf-8">\n' ...
    ...
    'th:first-child,td:first-child{text-align:left}code{background:#edf1f7;padding:2px 4px}%s</style>\n' ...
    '</head>\n<body>\n<h1>LTS Correlation Visualization</h1>\n' ...
    '<p><strong>Simulation:</strong> <code>%s</code><br><strong>Reality:</strong> <code>%s</code></p>\n' ...
    '%s%s%s<div id="path" class="plot"></div>\n<div class="grid">\n' ...
    ...
    '</script>\n</body>\n</html>\n'], ...
    plotlyScript, sceneLinkStyle, ...
    escapeHtml(summary.sim_file), escapeHtml(summary.real_file), ...
    metricsHtml, warningsHtml, sceneLinkHtml, ...
...
function html = sceneLink(sceneFile)
html = '';
if isempty(sceneFile)
    return;
end
[~, name] = fileparts(sceneFile);
html = sprintf('<p><a class="sceneLink" href="%s">&#9654; Open 3D replay (%s)</a></p>', ...
